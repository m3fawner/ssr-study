...
const getSSTaxableProceeds = (ssEarnings: CastedFormData['ssEarnings'] = 0, proceeds: CalculatedData['grantProceeds'] = 0) => {
...
const LineItem = ({ label, amount }: LineItemProps) => (
...
const HelpItem = ({ label, body }: HelpItemProps) => (
...
const EstimatedNetGrant = () => {
  ...
  const {
    ...
    netProceeds = 0,
    sharesToCover = 0,
  } = useMemo<CalculatedData>(() => {
    try {
      const casted: CastedFormData = schema.cast(watched) ?? {};
      const safePrice = currentPrice ?? 0;
      const proceeds = ((casted.sharesGranted ?? 0) * (safePrice));
      const ssTaxableProceeds = getSSTaxableProceeds((casted.ssEarnings ?? 0), proceeds);
      const state = ((casted.stateTax ?? 0) / 100) * proceeds;
      const federal = SUPPLEMENTAL_INCOME_RATE * proceeds;
      const medicare = MEDICARE_RATE * proceeds;
      const ss = ssTaxableProceeds * SS_TAX;
      ...
      return {
        ...
        sharesToCover: Math.ceil(totalTaxes / safePrice),
      };
    } catch (e) {
      return {};
    }
