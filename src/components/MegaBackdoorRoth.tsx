...
type AfterTaxSpaceCalculatorProps = {
  onSubmit: (formData: FormData) => void
}
const AfterTaxSpaceCalculator = ({ onSubmit }: AfterTaxSpaceCalculatorProps) => {
  const [afterTaxLimit, setAfterTaxLimit] = useState<number|null>(null);
...
type PerPaycheckProps = FormData;
const PerPaycheck = ({ limit = 0, employeeContributions = 0 }: PerPaycheckProps) => {
...
const MegaBackdoorRoth = () => {
  const [{ limit = 0, employeeContributions = 0 } = {}, setCalculatedAfterTaxMax] = useState<FormData|undefined>(undefined);
  return (
    <>
